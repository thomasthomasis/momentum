// All icons use fill="currentColor" or stroke="currentColor" so they inherit
// whatever CSS color is set on their container — works correctly in both themes.
// Pass className or size to override per-use.
...
export function EnergyIcon({ size = 14, className = '' }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden>
      <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />
    </svg>
...
// Energy level indicator — 5 small bars like a signal/battery meter
export function EnergyBars({ level }: { level: number }) {
  return (
    <span className="energy-bars" aria-label={`Energy ${level}/5`}>
      {[1, 2, 3, 4, 5].map(i => (
        <span
          key={i}
